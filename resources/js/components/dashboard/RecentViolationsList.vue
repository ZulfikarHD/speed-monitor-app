 ...
 *
 * Features:
 * - Ranked list with SVG medal icons for top 3
 * - Gradient badges for visual ranking
 * - Employee name + violation count
 * - Empty state with SVG icon
 * - Lightweight stagger animations
 * - Full light/dark theme support
 */

import { CheckCircle, Medal } from '@lucide/vue';
...
function getRankColorClasses(rank: number): string {
    const colorMap: Record<number, string> = {
        ...
        4: 'bg-gradient-to-r from-yellow-400 to-yellow-600 text-zinc-900',
        5: 'bg-gradient-to-r from-blue-500 to-indigo-500 text-white',
    };

    return colorMap[rank] || 'bg-zinc-200 dark:bg-zinc-700 text-zinc-900 dark:text-white';
}
</script>
    ...
    <div class="overflow-hidden rounded-lg border border-zinc-200 dark:border-white/5 bg-white/95 dark:bg-zinc-900/50 ring-1 ring-white/20 dark:ring-white/5 shadow-lg shadow-zinc-900/5 dark:shadow-cyan-500/5">
        ...
        >
            ...
            >
                <div class="flex items-center gap-4">
                    <div class="h-10 w-10 flex-shrink-0 animate-pulse rounded-full bg-zinc-200 dark:bg-zinc-700"></div>
                    <div class="flex-1 space-y-2">
                        <div class="h-4 w-1/2 animate-pulse rounded bg-zinc-200 dark:bg-zinc-700"></div>
                        <div class="h-3 w-1/3 animate-pulse rounded bg-zinc-200 dark:bg-zinc-700"></div>
                    </div>
                    <div class="h-8 w-16 animate-pulse rounded-full bg-zinc-200 dark:bg-zinc-700"></div>
                </div>
            </div>
        </div>
        ...
        >
            ...
            >
                <CheckCircle
                    :size="48"
                    :stroke-width="1.5"
                    class="mx-auto mb-4 text-emerald-500 dark:text-emerald-400"
                    aria-hidden="true"
                />
        ...
        >
            <motion.div
                ...
                class="px-6 py-4 transition-colors duration-200 hover:bg-zinc-50 dark:hover:bg-white/5"
            >
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <span
                            class="inline-flex items-center rounded-full border border-red-500/30 bg-red-500/20 dark:bg-red-500/15 px-3 py-1 text-sm font-semibold text-red-700 dark:text-red-400"
